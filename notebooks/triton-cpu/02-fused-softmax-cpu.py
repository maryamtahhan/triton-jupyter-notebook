...
def benchmark(M, N, provider):
    ...
    if provider == "torch-cpu-compile":
        ...
        ms, min_ms, max_ms = triton.testing.do_bench(
            lambda: compiled(x), quantiles=quantiles
        )
    if provider == "triton-cpu-single":
        ms, min_ms, max_ms = triton.testing.do_bench(
            lambda: softmax(x, y, num_threads=1), quantiles=quantiles
        )
    if provider == "triton-cpu":
        ms, min_ms, max_ms = triton.testing.do_bench(
            lambda: softmax(x, y), quantiles=quantiles
        )
