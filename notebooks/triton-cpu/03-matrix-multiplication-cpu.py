...
#
# ![grouped_vs_row_major_ordering](./grouped_vs_row_major_ordering.png)
#
...
if torch.allclose(triton_output, torch_output, atol=1e-2, rtol=rtol):
    ...
    print(
        "❌ TritonCPU and TorchCPU differ, the maximum difference is "
...
if USE_GPU and triton.runtime.driver.get_active_gpus():
    ...
    if torch.allclose(triton_output, torch_output, atol=1e-2, rtol=rtol):
        ...
        print(
            "❌ TritonGPU and TorchGPU differ, the maximum difference is "
            f"{torch.max(torch.abs(triton_output - torch_output))}"
        )
...
def benchmark(M, N, K, provider):
    ...
    if provider == "torch-gpu":
        ...
        ms, min_ms, max_ms = triton.testing.do_bench(
            lambda: matmul(a, b, None), quantiles=quantiles
        )
        ...
        ms, min_ms, max_ms = triton.testing.do_bench(
            lambda: compiled(a, b, out=c), quantiles=quantiles
        )
    elif provider == "triton-cpu-single":
        ms, min_ms, max_ms = triton.testing.do_bench(
            lambda: matmul(a, b, c, num_threads=1), quantiles=quantiles
        )
